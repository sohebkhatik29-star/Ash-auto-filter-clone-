...
async def handle_fsub_callbacks(client, query, data, user_id, r, save_fn, cancel_listeners_fn, edit_or_reply_fn, *args, **kwargs):
    ...
    async with lock:
        ...
        # 5. Add Channel Flow (Edits panel in-place so no multiple panels can stack)
        if data in ("m_add_fsub", "cset_fsub_add"):
            ...
            cancel_btn = InlineKeyboardMarkup([[InlineKeyboardButton("≼ CANCEL", callback_data="cset_fsub_menu")]])
            
            # Edit in place
            try:
                await edit_or_reply_fn(query, prompt_msg_text, reply_markup=cancel_btn)
                panel_msg_id = query.message.id if query.message else None
            except Exception:
                prompt_msg = await client.send_message(chat_id=user_id, text=prompt_msg_text, reply_markup=cancel_btn)
                panel_msg_id = prompt_msg.id
                track_fsub_msg(user_id, panel_msg_id)
            ...
            async def _add_fsub_worker():
                ...
                if not chat_id:
                    ...
                    try:
                        await client.edit_message_text(
                            chat_id=user_id,
                            message_id=panel_msg_id,
                            text="❌ <b>Please forward a message directly from the channel!</b>",
                            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("≼ BACK", callback_data="cset_fsub_menu")]])
                        )
                    except Exception:
                        m = await client.send_message(
                ...
                try:
                    await client.edit_message_text(
                        ...
                        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("≼ CANCEL", callback_data="cset_fsub_menu")]])
                    )
                except Exception:
                    pass
                ...
                mode_prompt_msg = await client.send_message(
                    chat_id=user_id,
                    text="<i>Choose mode from the keyboard below:</i>",
                ...
                while is_user_session_active(user_id, sess_token):
                    ...
                    if "request" in m_txt:
                        ...
                        mode_prompt_msg = await client.send_message(chat_id=user_id, text="⚠️ <i>Please choose Normal or Join Request.</i>", reply_markup=mode_kb)
                        track_fsub_msg(user_id, mode_prompt_msg.id)
        ...
        # 8. Set Custom Force Subscribe Text (Edits in-place)
        if data == "cset_fsub_set_text":
            ...
            cancel_btn = InlineKeyboardMarkup([[InlineKeyboardButton("≼ CANCEL", callback_data="cset_fsub_text_menu")]])
        ...
        if data == "cset_fsub_set_pic":
            ...
            cancel_btn = InlineKeyboardMarkup([[InlineKeyboardButton("≼ CANCEL", callback_data="cset_fsub_pic_menu")]])
