...
async def genlink(client, message):
    ...
    rec = bot_record(client); protected = bool(rec.get("protect_content", False)) or bool(rec.get("no_forward", False))
    username = (await client.get_me()).username; link = make_file_link(username, file_id, protected)
    user = await get_user(owner_id(client) or message.from_user.id); short = await get_short_link(user, link)
...
async def callbacks(client, query):
    ...
    if data=="protect_menu":
        rec=bot_record(client); status="Enabled ✅" if rec.get("protect_content") else "Disabled ❌"
