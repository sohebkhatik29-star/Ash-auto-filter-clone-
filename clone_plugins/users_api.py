...
def _ensure_daily_reset(bot_id: int, slot: int = 1):
    """Reset verified_today_{slot} if the stored date is not today (IST)."""
    if mongo_db is None:
        return
    try:
        today = _today_str_ist()
        date_key = f"verified_date_{slot}"
        count_key = f"verified_today_{slot}"
        rec = mongo_db.bots.find_one({"bot_id": int(bot_id)}, {date_key: 1, count_key: 1})
        if not rec:
            return
        stored_date = rec.get(date_key)
        if stored_date != today:
            mongo_db.bots.update_one(
                {"bot_id": int(bot_id)},
                {"$set": {count_key: 0, date_key: today}}
            )
    except Exception:
        pass


def get_verified_today(bot_id: int, slot: int = 1) -> int:
    """Return today's verified count for a bot/slot (auto-resets if date changed)."""
    if mongo_db is None:
        return 0
    try:
        _ensure_daily_reset(bot_id, slot)
        rec = mongo_db.bots.find_one({"bot_id": int(bot_id)}, {f"verified_today_{slot}": 1})
        return int((rec or {}).get(f"verified_today_{slot}", 0))
    except Exception:
        return 0
...
def set_user_verified(user_id: int, bot_id=0, duration_minutes=1440, slot=1):
    ...
    # Ensure daily reset then increment today's stats for this bot and slot
    try:
        _ensure_daily_reset(bot_id, slot)
        today = _today_str_ist()
        mongo_db.bots.update_one(
            {"bot_id": int(bot_id)},
            {
                ...
                "$set": {f"verified_date_{slot}": today}
            },
            upsert=True
        )
    except Exception:
...
def format_caption(custom_caption: str, media=None, source_msg=None, default_caption=None) -> str:
    ...
    res = (
        ...
        .replace("{size}", file_size)
        .replace("{FILE_SIZE}", file_size)
        .replace("{originalcaption}", orig_caption)
        .replace("{original_caption}", orig_caption)
        .replace("{ORIGINALCAPTION}", orig_caption)
