...
def get_bot(bid):
    return mongo_db.bots.find_one({"bot_id": int(bid)}) if mongo_db is not None else None


def update_bot(bid, **values):
    if mongo_db is not None:
        mongo_db.bots.update_one({"bot_id": int(bid)}, {"$set": values}, upsert=False)
...
async def clone_manage_action(client, query):
    ...
    d = get_bot(bid) or {}
    back = action_back(bid)

    if action == "startmsg":
        text = "📝 <b>START MESSAGE</b>\n\n" + (d.get("start_message") or "Not configured.")
    elif action == "force":
        channels = d.get("force_channels", [])
        text = f"📢 <b>FORCE SUB</b>\n\nConfigured channels: <code>{len(channels)}</code>\n\nUse /force_sub @channel from the selected clone to add a channel."
    elif action == "mods":
        mods = d.get("moderators", [])
        text = "👮 <b>MODERATORS</b>\n\n" + (("\n".join(f"• <code>{x}</code>" for x in mods)) or "No moderators configured.")
    elif action == "autodelete":
        text = f"🗑️ <b>AUTO DELETE</b>\n\nStatus: {'ON' if d.get('auto_delete_enabled') else 'OFF'}\nTime: {d.get('auto_delete_minutes', 15)} minutes"
    elif action == "noforward":
        text = f"🚫 <b>NO FORWARD</b>\n\nStatus: {'ON' if d.get('no_forward') else 'OFF'}"
    elif action == "access":
        text = f"🔑 <b>ACCESS TOKEN</b>\n\nStatus: {'ON' if d.get('access_token_enabled', True) else 'OFF'}\nValidity: {d.get('access_token_hours', 1)} hour(s)"
    elif action == "transfer":
        text = "🔄 <b>TRANSFER DB</b>\n\nDatabase transfer must be confirmed before changing ownership."
    elif action == "deactivate":
        new_state = not bool(d.get("deactivated"))
        update_bot(bid, deactivated=new_state)
        text = f"⏸ <b>DEACTIVATE</b>\n\nStatus changed to: {'DEACTIVATED' if new_state else 'ACTIVE'}"
    elif action == "mode":
        text = f"🔒 <b>MODE</b>\n\nCurrent mode: <code>{d.get('mode', 'private')}</code>"
    elif action == "restart":
        update_bot(bid, restart_requested=True)
        text = "🔄 <b>Restart requested.</b>\n\nThe selected clone will pick this up on its next manager check/restart."
    elif action == "stats":
        text = f"📊 <b>CLONE STATS</b>\n\nBot ID: <code>{bid}</code>\nOwner ID: <code>{d.get('user_id', 'Unknown')}</code>\nUsers: <code>{d.get('user_count', 0)}</code>"
    else:
        text = "⚠️ <b>DELETE CLONE</b>\n\nThis removes the clone record from the manager database."
        back = InlineKeyboardMarkup([[InlineKeyboardButton("YES, DELETE", callback_data=f"cmdelete:{bid}"), InlineKeyboardButton("CANCEL", callback_data=f"manage_clone:{bid}")]])
...
async def clone_delete(client, query):
    ...
    if not owns(query.from_user.id, bid):
        return await query.answer("❌ Access denied.", show_alert=True)
    mongo_db.bots.delete_one({"bot_id": bid})
